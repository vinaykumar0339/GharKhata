...
export default function Expenses() {
  ...
  const fundingSources = useMaster(selected?.id, 'fundingSources');
  ...
  const fundingSource = (id?: string) => fundingSources.find((item) => item.id === id)?.name ?? 'Unassigned source';
  ...
  return <Screen>
    ...
    {showFilter ? <Card style={styles.filters}>
      ...
      <Selector label="Funding source" value={filters.fundingSourceId} options={fundingSources} onChange={(fundingSourceId) => setFilters((current) => ({ ...current, fundingSourceId }))} />
