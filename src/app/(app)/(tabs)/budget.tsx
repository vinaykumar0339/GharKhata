...
import { budgetRows, budgetTypeTotals, expenseTotal } from '@/lib/analytics';
...
export default function BudgetScreen() {
  ...
  const budgetTypes = budgetTypeTotals(expenses, budget, project.totalBudget);
  ...
  return <Screen><Header title="Budget" subtitle={project.status === 'archived' ? 'Archived project · view only.' : 'Construction and other costs, clearly separated.'} right={<ProjectPicker projects={projects} />} />
    {total ? <><Card style={isOverBudget ? styles.overrunSummary : styles.summary}><Text style={styles.label}>{isOverBudget ? 'TOTAL BUDGET EXCEEDED' : 'TOTAL AVAILABLE TO SPEND'}</Text><Text style={styles.remaining}>{isOverBudget ? `${formatCurrency(overrun, currency)} over` : formatCurrency(Math.max(total - totalSpent, 0), currency)}</Text><View style={styles.summaryLine}><Text style={styles.spent}>Spent {formatCurrency(totalSpent, currency)}</Text><Text style={styles.spent}>of {formatCurrency(total, currency)}</Text></View><View style={styles.track}><View style={[styles.fill, isOverBudget && styles.overrunFill, { width: `${Math.min((totalSpent / total) * 100, 100)}%` }]} /></View></Card>
      <SectionTitle title="Where money is going" action="By budget type" />
      <Card style={styles.typeRows}>{budgetTypes.map((type) => <BudgetTypeRow key={type.bucket} {...type} currency={currency} />)}</Card>
    ...
    {budget ? <AuditText updatedBy={budget.updatedBy} /> : null}{total ? <><SectionTitle title="Construction: where money goes" action={constructionRows.length ? 'By category' : undefined} /><CategoryRows rows={constructionRows} empty="No construction category allocations yet. Add them to see precisely where construction money is going." currency={currency} />
      <SectionTitle title="Other project costs: where money goes" action={otherRows.length ? 'By category' : undefined} /><CategoryRows rows={otherRows} empty="No other-cost allocations yet. Add approvals, consultants, furnishings, or similar categories to see where this money is going." currency={currency} />
      {canPlan ? <Button variant="secondary" onPress={setBudget}>Edit budget</Button> : null}</> : null}
  </Screen>;
}

function BudgetTypeRow({ name, planned, actual, difference, currency }: { name: string; planned: number; actual: number; difference: number; currency: 'INR' | 'USD' }) {
  const over = difference < 0; const usage = planned ? Math.round((actual / planned) * 100) : 0;
  return <View style={styles.typeRow}><View style={styles.typeHeading}><Text style={styles.typeName}>{name}</Text><Text style={styles.typeActual}>{formatCurrency(actual, currency)} spent</Text></View><Text style={styles.typeCaption}>{planned ? `${formatCurrency(planned, currency)} planned · ${over ? `${formatCurrency(-difference, currency)} over` : `${formatCurrency(difference, currency)} left`}` : 'No budget set for this type'}</Text><View style={styles.typeTrack}><View style={[styles.typeFill, over && styles.typeFillOver, { width: `${Math.min(usage, 100)}%` }]} /></View></View>;
}
...
const styles = StyleSheet.create({ summary: { backgroundColor: colors.forest, borderColor: colors.forest, gap: 9 }, overrunSummary: { backgroundColor: colors.coral, borderColor: colors.coral, gap: 9 }, label: { color: '#FFF6F3', fontSize: 11, letterSpacing: .9, fontWeight: '900' }, remaining: { color: 'white', fontSize: 32, fontWeight: '900' }, summaryLine: { flexDirection: 'row', justifyContent: 'space-between' }, spent: { color: '#FFF6F3', fontSize: 12, fontWeight: '700' }, track: { height: 7, borderRadius: 4, backgroundColor: 'rgba(255,255,255,.34)', overflow: 'hidden' }, fill: { height: '100%', backgroundColor: colors.lime }, overrunFill: { backgroundColor: '#8B2E1F' }, typeRows: { gap: 18 }, typeRow: { gap: 7 }, typeHeading: { flexDirection: 'row', justifyContent: 'space-between', gap: 10 }, typeName: { color: colors.ink, fontSize: 15, fontWeight: '900', flex: 1 }, typeActual: { color: colors.ink, fontSize: 13, fontWeight: '900' }, typeCaption: { color: colors.muted, fontSize: 12, lineHeight: 17 }, typeTrack: { height: 8, backgroundColor: '#EEF3EF', borderRadius: 8, overflow: 'hidden' }, typeFill: { height: '100%', backgroundColor: colors.forest, borderRadius: 8 }, typeFillOver: { backgroundColor: colors.coral }, warning: { backgroundColor: '#FFF0EB', borderColor: '#F5B6A8', gap: 8 }, warningTitle: { color: '#8B2E1F', fontSize: 16, fontWeight: '900' }, warningText: { color: colors.ink, lineHeight: 19, fontSize: 13 }, rows: { gap: 16 }, row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 10, width: '100%' }, rowInfo: { flex: 1, flexShrink: 1, minWidth: 0 }, rowPill: { flexShrink: 1, maxWidth: '42%' }, rowName: { color: colors.ink, fontWeight: '900', fontSize: 15 }, rowSub: { color: colors.muted, fontSize: 12, marginTop: 3 }, emptyText: { color: colors.muted, lineHeight: 20 } });
