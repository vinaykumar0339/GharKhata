...
export const masterRepository = {
  ...
  create(ownerId: string, projectId: string, type: MasterType, name: string, actorName: string) { return createOwned<MasterItem>(type, { ownerId, projectId, type, name: name.trim(), createdBy: ownerId, createdByName: actorName, updatedBy: ownerId, updatedByName: actorName }); },
  update(userId: string, actorName: string, type: MasterType, id: string, name: string) { return updateDoc(doc(db, type, id), { name: name.trim(), updatedBy: userId, updatedByName: actorName, updatedAt: new Date().toISOString() }); },
  ...
  async seed(ownerId: string, projectId: string, actorName = 'GharKhata') { await Promise.all((Object.keys(defaults) as MasterType[]).map(async (type) => { const current = await this.list(projectId, type); if (!current.length) await Promise.all(defaults[type].map((name) => createOwned<MasterItem>(type, { ownerId, projectId, type, name, createdBy: ownerId, createdByName: actorName, updatedBy: ownerId, updatedByName: actorName }))); })); },
};
