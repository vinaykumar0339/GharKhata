...
export const profileRepository = {
  ...
  async ensure(userId: string, email: string, displayName: string) {
    const profile = doc(db, 'users', userId);
    if (!(await getDoc(profile)).exists()) await setDoc(profile, { email, displayName, currency: 'INR', createdAt: new Date().toISOString() });
  },
